...
const createSubscription = async (req, res, next) => {
    try {
        ...
        if (plans.length === 0) return errorResponse(res, 'Plan tidak ditemukan atau tidak aktif.', 404);
        const plan = plans[0];

        // Cek user sudah punya subscription aktif
        const [existingSubs] = await pool.query(
            'SELECT id FROM subscriptions WHERE user_id = ? AND status IN ("trial", "active")', [user_id]
        );
        if (existingSubs.length > 0) {
            return errorResponse(res, 'Anda sudah memiliki subscription aktif. Silakan upgrade atau cancel dulu.', 409);
        }
        ...
        const status = plan.trial_days > 0 ? 'trial' : (parseFloat(plan.price) === 0 ? 'active' : 'active');
        ...
        const [newSubs] = await pool.query(
            'SELECT * FROM subscriptions WHERE user_id = ? ORDER BY started_at DESC LIMIT 1', [user_id]
        );
        const subscription = newSubs[0];

        // Auto generate invoice
        const invoice = await createAutoInvoice({
            user_id, subscription_id: subscription.id, plan, periodStart, periodEnd
        });
        ...
        // Notifikasi
        await createNotification(user_id, 'Subscription Dibuat! 🎉',
            `Anda berhasil subscribe paket ${plan.name}. ${parseFloat(plan.price) > 0 ? 'Silakan selesaikan pembayaran.' : 'Selamat menggunakan!'}`,
            'success', 'subscription', { subscription_id: subscription.id }
        );

        return successResponse(res, { subscription, invoice }, 'Subscription berhasil dibuat!', 201);
...
const getMySubscription = async (req, res, next) => {
    try {
        const [subs] = await pool.query(`
            ...
            WHERE s.user_id = ? ORDER BY s.started_at DESC`, [req.user.id]
        );
...
const cancelSubscription = async (req, res, next) => {
    try {
        ...
        if (subs.length === 0) return errorResponse(res, 'Subscription tidak ditemukan.', 404);

        const sub = subs[0];
        if (user_role !== 'admin' && sub.user_id !== user_id) return errorResponse(res, 'Akses ditolak.', 403);

        const oldStatus = sub.status;
        await pool.query(
            'UPDATE subscriptions SET status = "cancelled", cancel_at_period_end = TRUE, cancelled_at = NOW() WHERE id = ?', [id]
        );

        await pool.query(`
            INSERT INTO subscription_history (subscription_id, user_id, action, old_status, new_status)
            VALUES (?, ?, 'cancelled', ?, 'cancelled')`, [id, sub.user_id, oldStatus]
        );

        await createNotification(sub.user_id, 'Subscription Dibatalkan',
            'Langganan Anda telah dibatalkan.', 'warning', 'subscription', { subscription_id: id }
        );
...
const previewChangePlan = async (req, res, next) => {
    try {
        ...
        if (!new_plan_id) return errorResponse(res, 'New plan ID wajib diisi!', 400);

        // Ambil subscription aktif
        const [currentSubs] = await pool.query(`
            SELECT s.*, p.name as plan_name, p.price, p.billing_cycle
            FROM subscriptions s JOIN plans p ON s.plan_id = p.id
            WHERE s.user_id = ? AND s.status IN ('active', 'trial') LIMIT 1`, [user_id]
        );
        if (currentSubs.length === 0) return errorResponse(res, 'Anda tidak memiliki subscription aktif.', 404);

        const currentSub = currentSubs[0];
        if (currentSub.plan_id === new_plan_id) return errorResponse(res, 'Anda sudah menggunakan paket ini.', 400);

        // Ambil plan baru
        const [newPlans] = await pool.query('SELECT * FROM plans WHERE id = ? AND is_active = TRUE', [new_plan_id]);
        if (newPlans.length === 0) return errorResponse(res, 'Plan baru tidak ditemukan.', 404);

        const currentPlan = { name: currentSub.plan_name, price: currentSub.price, billing_cycle: currentSub.billing_cycle };
        const newPlan = newPlans[0];
        const proration = calculateProration(currentPlan, newPlan, currentSub.current_period_end);
...
const changePlan = async (req, res, next) => {
    try {
        ...
        try {
            ...
            if (newPlan.billing_cycle === 'monthly') newPeriodEnd.setMonth(newPeriodEnd.getMonth() + 1);
            else if (newPlan.billing_cycle === 'yearly') newPeriodEnd.setFullYear(newPeriodEnd.getFullYear() + 1);
            ...
            await connection.query(`
                ...
                WHERE id = ?`, [new_plan_id, now, newPeriodEnd, currentSub.id]
            );
            ...
            await connection.query(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, 'active', ?)`,
                [currentSub.id, user_id, action, oldPlanId, new_plan_id, oldStatus, JSON.stringify(proration)]
            );
            ...
            const invoice = await createAutoInvoice({
                user_id, subscription_id: currentSub.id, plan: newPlan,
                periodStart: now, periodEnd: newPeriodEnd,
            ...
            await createNotification(user_id,
                proration.is_upgrade ? 'Paket Berhasil Di-upgrade! 🚀' : 'Paket Berhasil Di-downgrade',
                proration.summary, proration.is_upgrade ? 'success' : 'info', 'subscription',
                { subscription_id: currentSub.id, proration }
            );

            const [updatedSub] = await pool.query(`
                SELECT s.*, p.name as plan_name, p.price, p.billing_cycle
                FROM subscriptions s JOIN plans p ON s.plan_id = p.id WHERE s.id = ?`, [currentSub.id]
            );

            return successResponse(res, { subscription: updatedSub[0], invoice, proration }, `Paket berhasil di-${action}!`);
        } catch (err) {
...
const getSubscriptionHistory = async (req, res, next) => {
    try {
        ...
        return successResponse(res, history, 'Riwayat subscription berhasil diambil.');
    } catch (error) { next(error); }
};

module.exports = {
    createSubscription, getMySubscription, getAllSubscriptions, cancelSubscription,
    previewChangePlan, changePlan, getSubscriptionHistory
};