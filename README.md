...
4. Setup Database
   ...
   Buka phpMyAdmin
   Buat database baru: `subscription_db`
   Jalankan server — tabel akan auto-create
   Opsi B: MySQL Cloud (filess.io)
   Daftar di filess.io
   Buat database MySQL baru
   Copy credential ke `.env`
   Deploy ke Vercel dan akses `/api/setup` untuk auto-create tabel
5. Run Server
   Development:
...
Login with default accounts to get tokens
Use `{{accessToken}}` in Authorization tab for protected endpoints
...
Setup filess.io Database:
Create MySQL database at filess.io
